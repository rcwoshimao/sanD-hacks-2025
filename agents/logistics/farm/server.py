...
logger = logging.getLogger("lungo.logistics.farm.server")
...
async def liveness_probe(request):
    ...
    try:
        ...
        return JSONResponse(
            {
                "error": "Timeout occurred while creating client. Shipper "
        ...
        return JSONResponse(
            {
                "error": f"Error occurred: {str(e)}. "
                         f"Shipper agent is not reachable."
            },
            status_code=500
        )
...
async def run_http_server(server):
    ...
    try:
        config = Config(app=app_, host="0.0.0.0", port=9093, loop="asyncio")
